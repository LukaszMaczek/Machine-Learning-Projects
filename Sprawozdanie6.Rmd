...
## a) SVM z jądrem liniowym
Hiperparametry:

1. C – współczynnik regularyzacji. Kontroluje kompromis między szerokością marginesu a liczbą błędów:

- małe C- większy margines, więcej błędów 
- duże C- mniejszy margines, model próbuje klasyfikować idealnie

Wpływ na wyniki (wg otrzymanych wartości):

- najlepsze Accuracy dla C = 0.1 (ok. 0.775)
- rosnące C nie poprawia wyraźnie jakości
- małe C zwiększa czułość (wykrywanie klasy 1), ale zmniejsza specyficzność

Wniosek: Jądro liniowe działa stabilnie, ale nie osiąga najwyższych wyników.


## b) SVM z jądrem wielomianowym

Hiperparametry: 

- C     – regularyzacja (jak w jądrze liniowym)
- degree – stopień wielomianu (większy stopień - bardziej złożone granice)
- gamma  – wpływ pojedynczych obserwacji; duże gamma = bardzo złączenie
- coef0  – przesunięcie wielomianu (reguluje krzywiznę)

Wpływ na wyniki:

- najlepsze modele osiągnęły Accuracy ok. 0.78–0.79
- wysokie gamma często przeuczało
- wyższy degree nie zawsze poprawiał wyniki
- model poly nie był lepszy od radialnego

Wniosek: Jądro wielomianowe daje poprawne wyniki, ale jest trudne w strojeniu i nie osiągnęło najlepszego rezultatu.

## c) SVM z jądrem radialnym (RBF)

 Hiperparametry:
 
- C – regularyzacja
- gamma – jak bardzo wpływa pojedynczy punkt: (małe gamma - gładka granica, duże gamma - bardzo dopasowana, ryzyko przeuczenia)

Wpływ na wyniki:

- najlepsze modele uzyskały najwyższą średnią (Sensitivity+Specificity)/2
- najlepsze gamma = 0.01 lub 0.001
- C od 0.5 do 3 dawało najlepszą równowagę

Wniosek: Jądro radialne okazało się najlepsze – najwyższa stabilność,najlepsza równowaga między czułością i specyficznością.



## Porównanie trzech jąder SVM
Najlepsze wyniki uzyskaliśmy:

- Accuracy: zwykle najwyższe dla SVM radialnego
- Średnia (Sensitivity + Specificity) / 2 - radial również najlepszy

Ostateczny wniosek:
*Model SVM z jądrem radialnym (RBF)* był najlepszy spośród trzech, miał najwyższy Accuracy oraz najlepszą równowagę między czułością i specyficznością.
...
Porównanie z modelem prostym-regresja logistyczna

Wynik regresji logistycznej:

- Accuracy = 0.76
- Sensitivity i Specificity gorsze niż najlepsze modele SVM

Porównanie:

- regresja logistyczna działa poprawnie, ale nie dorównuje najlepszym modelom SVM (szczególnie radialnemu)

Wniosek końcowy: SVM z jądrem radialnym (RBF) był najlepszym modelem w zadaniu.
Prosty model (regresja logistyczna) osiągnął zauważalnie słabsze wyniki
