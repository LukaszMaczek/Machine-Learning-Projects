...
varImpPlot(rf_best)
vip(rf_best)
#Zmienna gender i Residence_type negatywnie wpływają na model, największy wpływ ma wiek
```

# Zadanie 1 Odpowiedzi

1) Wpływ liczby drzew, liczby zmiennych mtry oraz nodesize na wyniki:
- Większa liczba drzew (ntree) zwiększa stabilność i dokładność modelu, ale do pewnego stopnia
- mtry = 3 dawało najwyższą dokładność— oznacza to, że model działa lepiej, gdy w podziale bierze pod uwagę małą liczbę cech (większa losowość -mniejsze przeuczenie)
- Większe nodesize zmniejsza złożoność drzew i obniża przeuczenie, ale zbyt duże pogarsza jakość – najlepsze okazało się nodesize = 10.

2) Najlepszy model:
ntree = 20, mtry = 3, nodesize = 10
Ten wariant miał najwyższe metryki testowe oraz dobry balans czułości i specyficzności.

3) Ocena ważności zmiennych:
- najważniejszą zmienną okazał się age, następnie avg_glucose_level oraz bmi
- zmienne gender oraz Residence_type miały wpływ bliski 0 (a nawet negatywny), więc model praktycznie ich nie używa.

4) Interpretacja:
Las losowy potwierdza zależności kliniczne: ryzyko udaru rośnie wraz z wiekiem, wysokim poziomem glukozy i BMI. Model radzi sobie lepiej niż pojedyncze drzewo i bagging.
...
1) Wpływ hiperparametrów na wyniki
- Niższe eta (szczególnie 0.01) dawało najlepsze wyniki testowe, ponieważ model uczył się wolniej i nie przeuczał się.
- Płytsze drzewa (max_depth=2) działały lepiej– głębsze powodowały przeuczenie.
- subsample=1 i colsample_bytree=1 pozwalały modelowi w pełni korzystać z danych.
- nrounds około 100 było optymalne–większa liczba nie poprawiała już wyników.

2) Najlepszy model boostingu:
- eta = 0.01
- max_depth = 2
- subsample = 1
- colsample_bytree = 1
...
- Boosting miał wyraźnie wyższą czułość niż inne algorytmy (najlepiej wykrywa osoby z udarem).
- Boosting jest bardziej wrażliwy na przypadki pozytywne, co jest korzystne w medycynie.

4) Ważność zmiennych:
- podobnie jak w lesie losowym: kluczowe były age, avg_glucose_level i bmi.
- struktura ważności była bardziej skupiona, bo boosting preferuje cechy o najwyższej mocy predykcyjnej.
...
Poniżej zaprezentuje wnioski:

1) Drzewo decyzyjne
- najprostszy model
- niska czułość, model lepszy jedynie od baggingu

2) Bagging
- model o najsłabszych wynikach
- redukcja wariancji, ale brak losowości więc jest ryzyko przeuczenia

3) Random forest
- lepsze zbalansowanie czułości i specyficzności
- świetnie radzi sobie z wieloma cechami
- najlepszy model spośród metod baggingowych, ale nie osiąga zdecydowanie lepszych wyników

4) Boosting(XGBoost)
- najwyższa czułość (najlepsze wykrywanie udarów)
- najlepszy model w całym porównaniu
- działa sekwencyjnie, poprawiając błędy poprzednich drzew

**Główne wnioski:**

Zatem oceniamy modele w efektywne w poniższej kolejności:

- Boosting > Random Forest > Decision Tree > Bagging
- Boosting najskuteczniej wykrywa przypadki pozytywne, co jest kluczowe w zastosowaniach medycznych
- W obu modelach (RF i Boosting) wiek pozostaje kluczowym predyktorem
